...
import { adminOrderQuerySchema, createOrderSchema, orderIdParamsSchema, updateOrderStatusSchema } from './order.schema';

export const orderRouter = Router();
orderRouter.use(requireAuth);
orderRouter.get('/', controller.listMine);
orderRouter.post('/', validateBody(createOrderSchema), controller.create);
orderRouter.get('/admin/all', requireRole('admin'), validateQuery(adminOrderQuerySchema), controller.listAll);
