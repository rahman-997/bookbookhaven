...
export default function Header() {
  ...
  return <header className="sticky top-0 z-50 border-b border-white/[.07] bg-[#060912]/80 backdrop-blur-2xl">
    <div className="page-shell flex h-[72px] items-center justify-between gap-4">
      <Link href="/" className="group flex shrink-0 items-center gap-3" aria-label="BookHaven home">
        <span className="grid h-10 w-10 place-items-center rounded-xl border border-amber-200/15 bg-gradient-to-br from-amber-200/15 to-violet-400/10 text-amber-200 shadow-lg shadow-black/20 transition group-hover:-rotate-3"><BookIcon size={20}/></span>
        <span className="text-lg font-black tracking-[-.035em]">Book<span className="text-amber-200">Haven</span></span>
      </Link>

      <nav className="hidden items-center rounded-2xl border border-white/[.07] bg-white/[.025] p-1 md:flex" aria-label="Primary navigation">
        {links.map(({ href, label }) => {
          const active = href === '/' ? pathname === '/' : pathname.startsWith(href);
          return <Link key={href} href={href} aria-current={active ? 'page' : undefined} className={`rounded-xl px-4 py-2 text-sm font-semibold transition ${active ? 'bg-white/[.07] text-white' : 'text-slate-400 hover:text-white'}`}>{label}</Link>;
        })}
      </nav>
