...
export async function POST(request: NextRequest) {
  if (isCrossSiteMutation(request)) {
    return NextResponse.json({ success: false, error: { code: 'CROSS_SITE_REQUEST_BLOCKED', message: 'Cross-site registration is not allowed' } }, { status: 403 });
  }

  try {
    const body = await request.text();
    const upstream = await fetch(`${internalApiUrl}/auth/register`, {
      method: 'POST',
    ...
    return NextResponse.json({ success: false, error: { code: 'BACKEND_UNAVAILABLE', message: 'Registration is temporarily unavailable' } }, { status: 503 });
  }
}
