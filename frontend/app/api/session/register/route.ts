...
export async function POST(request: NextRequest) {
  try {
    ...
    const upstream = await fetch(`${internalApiUrl}/auth/register`, { method: 'POST', headers: { 'content-type': 'application/json' }, body, cache: 'no-store', signal: AbortSignal.timeout(10_000) });
